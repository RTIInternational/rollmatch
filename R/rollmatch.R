...
#' 
#'
#' @param scored_data Input data with propensity scores labeled "score"
#' @param data_full The original dataset before reduce_data() was ran
...
#' @param vars The column names used in the propensity score algorithm
...
rollmatch <- function(scored_data, data_full, treat, tm, entry, id, vars, 
                      alpha = 0, weighted_pooled_stdev = FALSE, num_matches = 3,
                      replacement=TRUE) {

  run_checks_one(scored_data, tm, entry, id)
  
  run_checks_two(scored_data, alpha, weighted_pooled_stdev, num_matches,
                 replacement)
  ...
  comparison_pool <- create_comparison(scored_data, tm, entry, id, treat)
  # Trim pool based on specified caliper
  trimmed_pool <- trim_pool(alpha = alpha, data_pool = comparison_pool,
                            scored_data = scored_data,
                            weighted_pooled_stdev = weighted_pooled_stdev)
  
  # Using matching algorithm to find top matches
  matches <- create_matches(trimmed_pool, num_matches, replacement)
  ...
  first_output <- create_weights(matches, data, id)
  ...
  combined_output <- make_output(scored_data = scored_data,
                                 data_full = data_full, matches = matches,
                                 treat = treat, tm = tm, entry = entry,
                                 lookback = lookback)
  # Add balance table to the output
  output <- add_balance_table(reduced_data = reduced_data, vars = vars, tm = tm,
